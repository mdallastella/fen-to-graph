...
(fact "Transforming a FEN string into a board"
      (flatten (:pieces (fen/split-fen-string fen))) => board)

(facts "Creating pieces"
       (fact (fen/char->piece \p :e7) => (pieces/->Pawn "pawn" :black :e7))
       (fact (fen/char->piece \N :c1) => (pieces/->Knight "knight" :white :c1))
       (fact (fen/char->piece \P :e2) => (pieces/->Pawn "pawn" :white :e2))
       (fact (fen/char->piece \K :e1) => (pieces/->King "king" :white :e1))
       (fact (fen/char->piece \q :d8) => (pieces/->Queen "queen" :black :d8)))
